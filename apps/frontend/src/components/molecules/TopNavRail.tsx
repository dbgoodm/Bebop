...
export const TopNavRail: React.FC<TopNavRailProps> = ({
  ...
  return (
    <header
      id="top-nav-rail"
      style={{ backgroundColor: currentTheme.bgSurface, borderBottomColor: currentTheme.borderColor }}
    ...
    >
      <div id="nav-left-section" className="flex min-w-0 items-center gap-4 lg:gap-7">
        <button
          id="nav-brand"
          type="button"
        ...
        >
          <span
            ...
            style={{ backgroundColor: `${currentTheme.primary}15`, borderColor: `${currentTheme.primary}60` }}
          >
        ...
        <nav id="main-navigation" className="hidden min-w-0 items-center gap-3 font-mono text-xs sm:flex md:gap-5">
      ...
      <div id="nav-center-section" className="col-span-2 row-start-2 min-w-0 md:col-span-1 md:col-start-2 md:row-start-1">
        <label id="nav-search-container" className="relative block">
          ...
          <input
            ...
            className="w-full rounded-lg border py-2 pl-3 pr-9 text-sm text-neutral-100 placeholder:text-neutral-500 focus:outline-none"
          />
          {searchQuery ? (
            <button
              id="nav-search-clear"
            ...
            <Search className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2" style={{ color: currentTheme.primary }} />
          )}
        </label>
      </div>

      <div id="nav-right-section" className="flex items-center justify-end gap-1">
        {showPrototypeActions ? (
          <>
            <input ref={fileInputRef} type="file" accept="audio/*,.mp3,.flac,.wav,.ogg,.m4a" className="hidden" onChange={handleFileChange} />
            <button
              id="nav-import-audio-btn"
              type="button"
              onClick={() => fileInputRef.current?.click()}
              className="rounded-lg p-2 text-neutral-400 transition hover:bg-neutral-800/60 hover:text-neutral-100"
